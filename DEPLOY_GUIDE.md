...
# ถ้ามี remote "origin" ตัวอย่างผูกไว้ก่อนแล้ว ใช้ set-url แทน add
git remote set-url origin https://github.com/kppdoaedev2026-eng/cassavaplot-monitor.git
git push -u origin main
```
> ข้าม `git init` / `git branch -M main` ได้ถ้าโฟลเดอร์เป็น Git repo (branch `main`) อยู่แล้ว
...
> **สำคัญ:** ห้ามรัน `clasp create` ตรงในโฟลเดอร์ `gas/` — โฟลเดอร์นี้มี `.clasp.json` (ใส่ scriptId ตัวอย่างไว้) อยู่แล้ว `clasp create` จะปฏิเสธเพราะมีโปรเจกต์อยู่แล้ว ให้สร้างในโฟลเดอร์ว่างแยกต่างหาก (หรือสร้างผ่านเว็บ) แล้วเอา scriptId มาใส่แทน

```bash
mkdir /tmp/gas-create-temp && cd /tmp/gas-create-temp
clasp create --title "CassavaPlot Monitor v4"
cat .clasp.json   # copy ค่า scriptId ที่ได้
```

คำสั่งนี้จะสร้าง GAS project ใหม่ใน Google Drive และคืนค่า `scriptId` มาให้
...
จากนั้นแก้ `gas/.clasp.json` (ไฟล์เดิมที่มีอยู่แล้ว) ใส่ scriptId ที่ได้ — **อย่าลบ** `filePushOrder`:
```json
{
...
> **สำคัญ:** ต้องเลือก Source เป็น **"GitHub Actions"** เท่านั้น ห้ามเลือก "Deploy from a branch" — เพราะ `.github/workflows/deploy.yml` ใช้ `actions/configure-pages` + `actions/deploy-pages` ซึ่งต้องคู่กับ Source แบบ GitHub Actions เท่านั้น ถ้าเลือกผิดเป็น "Deploy from a branch" workflow จะ deploy ไม่สำเร็จ

1. ไปที่ GitHub repo → **Settings** → **Pages**
2. หัวข้อ **Build and deployment** → **Source** → เลือก **GitHub Actions**
3. ไม่ต้องเลือก branch/folder เพิ่ม — workflow จัดการเอง
4. รอให้ Actions รันสำเร็จครั้งแรก (ขั้นตอนที่ 8) → GitHub Pages URL จะแสดง เช่น:
   ```
...
### 8.2 บัญชี Admin (สร้างให้อัตโนมัติแล้ว)

`setupSystem()` สร้างบัญชี admin ให้อัตโนมัติในชีต **ผู้ใช้งาน** — ไม่ต้องเพิ่มเอง:

| เบอร์โทร | PIN |
|---------|-----|
| `0805165313` | `123651` |

> **ความปลอดภัย:** ค่านี้ฝัง (hardcode) อยู่ใน `Code.gs` ซึ่งเป็น public repo — ใครก็เห็นได้ ต้อง login แล้วเปลี่ยน PIN ทันที (ขั้นตอนที่ 9.1) และควรสร้างบัญชีเจ้าหน้าที่จริงเพิ่มภายหลัง
...
เปิด Web App URL → แท็บ **👔 เจ้าหน้าที่**
- เบอร์โทร: `0805165313`
- PIN: `123651`

ไปที่ **ตั้งค่า** → เปลี่ยน PIN ใหม่ทันที (ค่าเดิม hardcode อยู่ใน public repo)
